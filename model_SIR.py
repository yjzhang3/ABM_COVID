...
class PersonAgent(Agent):
    ...
    def __init__(self, unique_id, radius, prob, model):
        ...
        if self.infect_state == 1:
            self.model.num_I += 1
            self.model.num_S =self.model.num_agents-self.model.num_I
        """ calculate infected and uninfected in the whole system """

        print('initialized as ' + 'id: '+ str(self.unique_id) + ' infected? '+ str(self.infect_state)+'\n')
        print('total infected: ' + str(self.model.num_I)+'\n')
    ...
    def step(self):
        ...
        print('agent id ' + str(self.unique_id) + ' has the following neighbors: ' + '\n')
        ...
        if all_neighbors != [] and self.infect_state == 1: # if any other agent is in my infection radius and I'm infected
            neighbor_infect_state = []
            for other_agent in all_neighbors: # record a list of what the infection state is of my neighbors
                neighbor_infect_state.append(other_agent.infect_state)
                print('neighbor infected?', other_agent.infect_state)
            weights = [self.infect_prob,1-self.infect_prob] # probability of infecting others, not infecting others
            new_neighbor_infect_state = choices([1,0],weights = weights, k = len(all_neighbors))
...
class PopulationModel(Model):
    ...
    def __init__(self,N,r,p,width,height):
        ...
        for i in range(self.num_agents):
            a = PersonAgent(i,self.required_social_dist,self.infect_overallprob, self)
            # i is the id number
            self.schedule.add(a)
